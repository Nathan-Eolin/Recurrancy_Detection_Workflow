...
do
	...
	CHECK_FATE_G13=`egrep "${GENES[0]:1:20}|${GENES[2]:1:20}" $FCLUSTER_FILE | wc -l`
	CHECK_FATE_G14=`egrep "${GENES[0]:1:20}|${GENES[3]:1:20}" $FCLUSTER_FILE | wc -l`
	...
	PATTERNS=`cat $PATTERN_FILE | egrep "${GENES[0]:1:20}" | egrep "${GENES[2]:1:20}" | head -1 | cut -d " " -f5-`
