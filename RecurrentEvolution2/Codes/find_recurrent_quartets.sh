...
#make a statement that checks if both files are created and if they aren't then create an empty file
if [[ -f "$NON_REC_PAIRS_CAT1" ]];
then
	true
else
	touch $NON_REC_PAIRS_CAT1
fi;
if [[ -f "$NON_REC_PAIRS_CAT2" ]];
then
	true
else
	touch $NON_REC_PAIRS_CAT2
fi;
